...
```text
commercial-license-skill install                  # 대화형으로 에이전트 스킬 폴더에 설치
commercial-license-skill uninstall                # 등록된 에이전트 스킬 제거
commercial-license-skill doctor                   # 로컬 에이전트 설정 및 실행 환경 점검
commercial-license-skill scan [path]              # 특정 디렉토리 내 라이선스 위험성 스캔
commercial-license-skill recommend <pkg> [--online] # 허용형 라이선스 대체 패키지 추천
commercial-license-skill mcp                      # MCP 서버 구동
```
*더 많은 옵션과 상세한 옵션은 `commercial-license-skill --help` 명령어로 확인할 수 있습니다.*
